package com.ollamatest.service;
...
import com.ollamatest.model.HistoryEntry;
...
public class HelpDeskChatbotAgentService {
    ...
    public String call2(String userMessage, String historyId) {
        List<HistoryEntry> currentHistory = conversationalHistoryStorage.computeIfAbsent(historyId, k -> new ArrayList<>());
        ...
        UserMessage currentPromptMessage = new UserMessage(CURRENT_PROMPT_INSTRUCTIONS.concat(userMessage));

        Prompt prompt = new Prompt(List.of(generalInstructionsSystemMessage, contextSystemMessage, currentPromptMessage));
        String response = ollamaChatClient.call(prompt).getResult().getOutput().getText();
        HistoryEntry contextHistoryEntry = new HistoryEntry(userMessage, response);
//        ChatResponse response = ollamaChatClient.call(prompt);
//        String res = response.getResult().getOutput().getText();
//        var contextHistoryEntry = new HistoryEntry(userMessage, res);
        currentHistory.add(contextHistoryEntry);

        return response;
    }
    
    public String call3(String userMessage) {
    	 return ollamaChatClient.call(userMessage);
    }
}